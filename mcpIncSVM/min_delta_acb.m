...
if (polc == 1)  
   delta_m = -model.g(indc)/gamma(indc);
   nstatus(1) = MARGIN;   
   if (length(beta) > 1)   % if there are margin vectors 
      delta_e = model.C(indc) - model.a(indc);
      nstatus(2) = ERROR;   
   ...
   if (model.g(indc) > 0)        % decrementing indc when a(indc) > 0 and g(indc) > 0 during kernel perturbation
      delta_m = model.g(indc)/gamma(indc);
      nstatus(1) = MARGIN;
   else
      delta_m = Inf;
   end;
   if (model.a(indc) <= model.C(indc))
      delta_e = Inf;
   	  delta_r = model.a(indc);
      if (model.g(indc) > 0)     % decrementing indc when a(indc) > 0 and g(indc) > 0 during kernel perturbation
         nstatus(3) = RESERVE;
      else
         nstatus(3) = UNLEARNED;
      ...
      delta_e = model.a(indc) - model.C(indc);
      delta_r = Inf;
      nstatus(2) = ERROR;
   end;
end;
...
if (length(beta) > 1)  % if there are margin vectors  
   ...
   flags = (abs(beta_s) > 0);
   [delta_mer,i] = min_delta(flags,model.a(model.ind{MARGIN}),model.C(model.ind{MARGIN}).*(beta_s > 0),beta_s);
   if (delta_mer < Inf)
      indss(4) = model.ind{MARGIN}(i); 
      cstatus(4) = MARGIN;
...
gamma_e = polc*gamma(model.ind{ERROR});
flags = (gamma_e > 0);
[delta_em,i] = min_delta(flags,model.g(model.ind{ERROR}),zeros(length(model.ind{ERROR}),1),gamma_e);
if (delta_em < Inf)
   indss(5) = model.ind{ERROR}(i);
   cstatus(5) = ERROR;
   nstatus(5) = MARGIN;
end;
...
if (rflag)
   gamma_r = polc*gamma(model.ind{RESERVE});
   flags = (model.g(model.ind{RESERVE}) >= 0) & (gamma_r < 0);
   [delta_rm,i] = min_delta(flags,model.g(model.ind{RESERVE}),zeros(length(model.ind{RESERVE}),1),gamma_r);
   if (delta_rm < Inf)
      indss(6) = model.ind{RESERVE}(i);
      cstatus(6) = RESERVE;
      nstatus(6) = MARGIN;    
   end;
else
