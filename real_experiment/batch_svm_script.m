...
%     predict_bsv=[predict_bsv ones(size(predict_bsv,1),1)];
    ...
    disp(['Batch ',num2str(i),' sensitivity is : ',num2str(metrics.sensitivity)]);
    disp(['Batch ',num2str(i),' specificity is : ',num2str(metrics.specificity)]);
    disp(['Batch ',num2str(i),' F1_measure is : ',num2str(metrics.F1_measure)]);
...
%     F1=[F1;metrics.F1_measure];
    ...
    %% Add all anomaly and top 10% normal.
    [~,index]=sort(score,'descend');
    %anomaly_index=index(end-length(find(score<0)):end);
    anomaly_index=find(score<0);
    normal_index=index(1:floor(length(find(score>0))*0.1));
